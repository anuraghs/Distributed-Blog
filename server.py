...
sock_out3 = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
sock_out3.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)

sock_out4 = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
sock_out4.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
...
def broadcast_message(message, delay = MESSAGE_DELAY):
    global sock_out1, sock_out2, sock_out4, sock_out3, inputStreams
    helpers.broadcast_message(f"server {pid} -> {message}", [sock_out1, sock_out2, sock_out3, sock_out4], delay)
...
def handle_received_accept(received_ballot_num, received_accept_val, sender_pid, stream):
    ...
    if received_ballot_num >= ballot_num:
        ...
        if (len(blockchain) > 0 and block.hash != bc.hash_block(blockchain[-1])):
            # this server is out of sync and behind, ask leader for up-to-date blockchain
            print(block.hash)
            print(bc.hash_block(blockchain[-1]))
            reset_accept_val()
            reset_accept_num()
...
def server_communications(stream):
    global pid, blockchain, database, broken_streams, sock_in1, sock_out1, sock_out2, sock_out3, sock_out4, leader_pid, leader_stream, new_election, temporary_operations
    ...
    while True:
        ...
        if data:
            ...
            if sender == "server":
                ...
                if command == "crashed":
                    if pid == 1:
                        if payload_tokens[1] == 2:
                            sock_out1.close()
                        if payload_tokens[1] == 3:
                            sock_out2.close()
                        if payload_tokens[1] == 4:
                            sock_out3.close()
                        if payload_tokens[1] == 5:
                            sock_out4.close()
                    if pid == 2:
                        if payload_tokens[1] == 1:
                            sock_out1.close()
                        if payload_tokens[1] == 3:
                            sock_out2.close()
                        if payload_tokens[1] == 4:
                ...
                if command == "crash" or command == "failProcess":
                    helpers.handle_exit([sock_in1, sock_out1, sock_out2, sock_out3, sock_out4])
                    return
                ...
                if command == "prepare":
                    ...
                    if leader_pid == pid:
                        if payload_tokens[1] == "post":
                            def callback(block):
                                broadcast_message(f"resp{PAYLOAD_DELIMITER}{block.operation.op}{PAYLOAD_DELIMITER}{block.operation.username}{PAYLOAD_DELIMITER}{block.operation.title}{PAYLOAD_DELIMITER}{block.operation.content}")
                        else:
                            def callback(block):
                                broadcast_message(f"resp{PAYLOAD_DELIMITER}{block.operation.op}{PAYLOAD_DELIMITER}{block.operation.username}{PAYLOAD_DELIMITER}{block.operation.title}{PAYLOAD_DELIMITER}{block.operation.content}")
                        temporary_operations.put((bc.Operation(op=command, usr=payload_tokens[2], title=payload_tokens[3], content=payload_tokens[4]), callback))
...
def begin_paxos(proposed_operation, callback):
    ...
    if new_election and leader_pid == None:
...
def send_connections():
    out_addr1, out_addr2, out_addr3, out_addr4 = helpers.get_output_connection_tuples(pid, IP, port_base)
    sock_out1_result = None
    sock_out2_result = None
    sock_out3_result = None
    sock_out4_result = None
    while (sock_out1_result != 0 or sock_out2_result != 0 or sock_out3_result != 0 or sock_out4_result != 0):
        sock_out1_result = sock_out1.connect_ex(out_addr1)
        sock_out2_result = sock_out2.connect_ex(out_addr2)
        sock_out3_result = sock_out3.connect_ex(out_addr3)
        sock_out4_result = sock_out4.connect_ex(out_addr4)
        ...
        if sock_out3_result == 0:
            threading.Thread(target=server_communications,
                     args=(sock_out3,), daemon=True).start()
        if sock_out4_result == 0:
            threading.Thread(target=server_communications,
                     args=(sock_out4,), daemon=True).start()
        time.sleep(1)
...
def broadcast_command(input):
    global ballot_num, accept_val, leader_pid
    broadcast_message(f"command{PAYLOAD_DELIMITER}{input[0]}{PAYLOAD_DELIMITER}{input[1]}{PAYLOAD_DELIMITER}{input[2]}{PAYLOAD_DELIMITER}{input[3]}")
    timeout_time = time.time() + 5  # timeout after 5 seconds
    while not got_command:
        if time.time() >= timeout_time:
            ...
            if input[0] == "post":
                ...
                temporary_operations.put((bc.Operation(op=input[0], usr=input[1], title=input[2], content=input[3]), callback))
                return
            else:
                def callback(block):
                        broadcast_message(f"resp{PAYLOAD_DELIMITER}{block.operation.op}{PAYLOAD_DELIMITER}{block.operation.username}{PAYLOAD_DELIMITER}{block.operation.title}{PAYLOAD_DELIMITER}{block.operation.content}")
...
def broadcast_crash():
    broadcast_message(f"crashed{PAYLOAD_DELIMITER}{pid}")



def input_listener():
    global pid, blockchain, got_command
    user_input = input()
    while True:
        tokens = user_input.split(" ")
        command = tokens[0]
        if command == "crash" or command == "failProcess":
            threading.Thread(target=broadcast_crash,
                                    args=(), daemon=True).start()
            helpers.handle_exit([sock_in1, sock_out1, sock_out2, sock_out3, sock_out4])
            return
        elif command == "p" or command == "persist":
            ...
            if (bc.doesNotExist(blockchain, title)):
                if leader_pid == pid or leader_pid == None:
                    temporary_operations.put((bc.Operation(op=command, usr=username, title=title, content=content), callback))
                else:
                    broadcast_command([command, username, title, content])
                #temporary_operations.put((bc.Operation(op=command, usr=username, title=title, content=content), callback))
            else:
                log("Duplicate Title")
            ...
            if (not bc.doesNotExist(blockchain, title)):
                if leader_pid == None or leader_pid == pid:
                    temporary_operations.put((bc.Operation(op=command, usr=username, title=title, content=content), callback))
                else:
                    broadcast_command([command, username, title, content])
                #temporary_operations.put((bc.Operation(op=command, usr=username, title=title, content=content), callback))
            ...
            log(bc.print_blockchain(blockchain=blockchain))
        elif command == "view":
            username = tokens[1]
            log(bc.print_username(blockchain, username))
        elif command == "read":
            title = tokens[1]
            log(bc.print_title(blockchain, title))
        elif command == "queue":
            log(print(list(temporary_operations.queue)))
        user_input = input()
